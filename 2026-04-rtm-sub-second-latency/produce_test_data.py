...
def generate_block(block_number: int) -> dict:
    ...
    return {
        ...
        "block_hash": f"0x{random.randbytes(32).hex()}",
        "parent_hash": f"0x{random.randbytes(32).hex()}",
        "miner": f"0x{random.randbytes(20).hex()}",
        "gas_used": gas_used,
        "gas_limit": gas_limit,
...
def main():
    ...
    try:
        while True:
            ...
            # Send ~10 messages per second for testing
            time.sleep(0.1)
