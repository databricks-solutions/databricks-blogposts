...
# Try local PySpark first, fall back to Databricks Connect
import os

def get_spark_session():
    """Get Spark session - prefers local PySpark, falls back to Databricks Connect."""
    # Try local PySpark first
    try:
        from pyspark.sql import SparkSession
        spark = SparkSession.builder.appName("RTM_Guardrail_E2E_Test").getOrCreate()
        print("✅ Spark session created (local PySpark)")
        return spark
    except Exception:
        pass
    ...
    try:
        ...
        # Use profile from env var, or default to no profile (uses DATABRICKS_HOST/TOKEN)
        profile = os.environ.get("DATABRICKS_PROFILE")
        builder = DatabricksSession.builder.serverless(True)
        if profile:
            builder = builder.profile(profile)
        spark = builder.getOrCreate()
        print(f"✅ Spark session created (Databricks Connect - profile: {profile or 'default'})")
        return spark
...
test_blocks = [
    ...
    (1000010, "0xabc010", "0xparent010", "0x742d35Cc6634C0532925a3b844Bc9e7595f8dB10",
     5500000, 15000000, 110, 1710327054, "400000000000000000", f"Key: {TEST_AWS_KEY}"),
    ...
    (1000012, "0xabc012", "0xparent012", "0x742d35Cc6634C0532925a3b844Bc9e7595f8dB12",
     4500000, 15000000, 95, 1710327056, "250000000000000000", f"Token: {TEST_JWT}"),
]
