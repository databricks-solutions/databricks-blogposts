...
resource "azurerm_storage_account" "this" {
  ...
  network_rules {
    ...
    ip_rules = var.storage_account_allowed_ips // e.g., Terraform environment IP
    
    # List of allowed virtual network subnet IDs
    virtual_network_subnet_ids = concat(
      [for k, v in data.azurerm_subnet.ws_subnets : data.azurerm_subnet.ws_subnets[k].id],
      jsondecode(data.restapi_object.subnets.api_response).egress_config.default_rules.azure_service_endpoint_rule.subnets
    )
  }
